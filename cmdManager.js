...
const CMDS = {
    login: createCmd(['login', 'Comando di login', 'Benvenuto Agente %(name)s', 'Si è verificato un errore: %s']),
    logout: createCmd(['logout', 'Comando di logout', 'Arrivederci Agente %(name)s', 'Si è verificato un errore: %s']),
    ...
    agents: createCmd(['agents', 'Visualizza la lista degli agenti', 'Agenti registrati:\n\r%s', 'Si è verificato un errore: %s']),
...
const getCmd = (cmd) => {
    ...
    return cmdKey?CMDS[cmdKey]:null;
}
